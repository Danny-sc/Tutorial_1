...
We can now use the function `generate_new_design` to obtain a new set of points for the second wave. This function takes the trained emulator, the range of the parameter, the number of points to propose and the target name:

```{r}
new_points1d <- generate_new_design(em1d_1, 10, target1d, method = 'lhs', measure.method = 'maximin')
```
...
```{r, fig.width = 7, fig.height = 7}
new_new_points1d <- generate_new_design(c(em1d_2, em1d_1), 10, 
